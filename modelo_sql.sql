...
CREATE TABLE IF NOT EXISTS `cinema`.`TICKET` (
  ...
  INDEX `fk_TICKET_FUNCION1_idx` (`FUNCION_SALA_n_sala` ASC, `FUNCION_PELICULA_id_pelicula` ASC, `FUNCION_dia` ASC, `FUNCION_hora` ASC),
  ...
  CONSTRAINT `fk_TICKET_FUNCION1`
    FOREIGN KEY (`FUNCION_SALA_n_sala` , `FUNCION_PELICULA_id_pelicula` , `FUNCION_dia` , `FUNCION_hora`)
    REFERENCES `cinema`.`FUNCION` (`SALA_n_sala` , `PELICULA_id_pelicula` , `dia` , `hora`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cinema`.`TURNO` (
  `hora` INT NOT NULL,
  `PROYECTADOR_EMPLEADO_id_empleado` INT UNSIGNED NOT NULL,
  `SALA_n_sala` INT NOT NULL,
  ...
  CONSTRAINT `fk_TURNO_SALA1`
    FOREIGN KEY (`SALA_n_sala` , `SALA_CINE_id_cine`)
    REFERENCES `cinema`.`SALA` (`n_sala` , `CINE_id_cine`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
